...
void EpubGenerator::generate(const QList<XTAinfo> &listXta)
{
    ...
    save(epubFolder_,outputFolder_ + outputEpub_);
...
bool EpubGenerator::save(const QString &fullFolderPath, const QString &fullFilePath)
{
    QuaZip zip(fullFilePath);
    if (!zip.open(QuaZip::mdCreate)) {
        qDebug() << "Couldn't open " << fullFilePath;
        return false;
    }
    zip.setComment(QString("This epub was generated by TabZ-%1").arg("0.1.1"/*CURRENT_VERSION*/));

    //qDebug() << "Add folder " << fullFolderPath;

    QDirIterator it(fullFolderPath, QDir::NoDotAndDotDot | QDir::AllEntries, QDirIterator::Subdirectories);
    while (it.hasNext()) {
        ...
        newfilename.remove(fullFolderPath);

        //qDebug() << "Add " << newfilename;

        QuaZipFile tmpFile(&zip);
        if(!tmpFile.open(QIODevice::WriteOnly,
            QuaZipNewInfo(newfilename,filename))){
            //qDebug() << "Can't open file '" + filename + "' zip clone in write mode";
            return false;
        }
        QFile inFile(filename);
        if(!inFile.open(QIODevice::ReadOnly)){
            //qDebug() << "Can't open file '" + filename + "' in read mode";
            return false;
        }
        tmpFile.write(inFile.readAll());
        inFile.close();
        tmpFile.close();
    }
    zip.close();
